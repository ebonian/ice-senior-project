...
**E010 is the current iteration (operator go, 2026-09-03): the capital-parameterized venue re-screen.** The operator's capital amendment (north star, above) reopens the venue question three ways: (i) mechanical — restate E003/E006/E007/E008 at $10k (expected unchanged in sign: the frozen cost stack is bps-proportional except $0.0101/tx gas, so per-dollar verdicts are size-invariant — verify, don't assert); (ii) Arbitrum share-gates re-bind at 7× size (LINK/WETH's honest arm at 0.91% share → ~6.4%, expected to die; wstETH/WETH 0.28% → ~2%, carry unaffected); (iii) **the venue menu opens — Ethereum mainnet becomes calibratable**, carrying the named feeProtocol hypothesis: Arbitrum pools surrender 25% of fees (0x44), and if mainnet pools still run the fee switch off, the 1.33× fee-income multiplier maps E005's 0.86–0.97 near-misses to 0.87–1.29 — read on-chain, never assumed. Every venue is scored through **both lenses**: static-carry APR and model headroom (does any width arm clear f/g 1.0 — the question that decides whether the range/timing model thesis gets its venue back). The wstETH go/no-go (E009 SUPPORTED, margin design drafted) is **on hold pending E010** — it was framed against a closed menu at $1,420. Hypothesis card [B4](../discovery/BACKLOG.md); memo M004 first per the discovery loop. No capital moves on any of these numbers (bot ADR 0008).
