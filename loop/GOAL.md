...
**E009 SUPPORTED (2026-09-03) — B2's funding-persistence prerequisite is met.** Run at operator direction against the [M003](../discovery/memos/M003-eth-funding-persistence.md) pre-named estimator set: the trailing-12-month expected package net is **+$0.186/day (4.8% APR on $1,420)** and every pre-registered downside bound holds — worst rolling-30d −$0.086/day, longest negative-carry run 11 days, carry positive even in down-trend regimes, ~49% of hours pinned at HL's structural interest floor (38% of all carry). E005's measured carry was representative, and lean: half-year means compress from +$0.77/day (2024H1) to +$0.12/day (2026H1), and HL's history contains no full bear market — the Binance proxy's Merge-2022 worst 30d (−$0.58/day) would have breached the bound ([E009 report §4](../backtest_model_server/e009/REPORT.md)). **The loop remains ESCALATED per [PROTOCOL §7](PROTOCOL.md): what's left before any capital is an operator decision — whether a ~4.8%-APR-central package clears deployment against the 10% K1 floor — plus bot-side work: per-venue perp cost calibration and the K8 margin/liquidation design pass, whose stress case should budget a sustained −$0.60/day carry reversal (K12).** G2/item-F retrain stays blocked pending the operator's B2 call. No capital moves on any of these numbers (bot ADR 0008).
