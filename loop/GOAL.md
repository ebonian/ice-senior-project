...
**E007 is DONE — REFUTED (2026-09-02; [experiments/E007-causal-signal-test.md](experiments/E007-causal-signal-test.md)) — and the loop is ESCALATED to the operator on the venue decision, now three-way again.** The M001→E007 sequence closed both remaining capture routes on ETH/USDC 0.05%: the memo-phase constrained oracle showed daily-scale regime calls lose money even with perfect foresight (M001 §2), and the pre-registered causal-signal test refuted all six memo-ranked hour-scale candidates — every one negative over the full window and on held-out August at both arms, 0/540 tuning configurations positive anywhere. E006's conditional fourth option ("stay iff E007 finds a realizable filter clearing ~6–10% capture") **dies as specified**; the operator's call reverts to E005's three-way (accept a 5–7% funding-carry venue / re-screen another chain / judge the family), with one measured-but-untested residual on the table: streak-aware rules (hysteresis, dwell minimums, DP over a forecast payoff series) on a C3-quality selector — the E007 report §2 shows selection quality is achievable (AUC 0.62, positive-mean held set) and contiguity is what threshold rules cannot deliver. G2/item-F retrain stays blocked pending the venue decision. No capital moves on any of these numbers (bot ADR 0008). Per PROTOCOL §7 the loop stops here: the next action belongs to the operator.
