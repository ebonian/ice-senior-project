...
def run_ppo_training(data_dir: str, timesteps: int = 50000, num_envs: int = 4, initial_capital: float = 1000.0, save_prefix: str = "models"):
    """Train PPO model."""
    print("\n" + "=" * 60, flush=True)
...
def run_dqn_training(data_dir: str, episodes: int = 200, device: str = "cpu", initial_capital: float = 1000.0, save_prefix: str = "models"):
...
def run_lstm_dqn_training(data_dir: str, episodes: int = 1000, seq_len: int = 24, device: str = "cpu", initial_capital: float = 1000.0, save_prefix: str = "models"):
...
def evaluate_ppo(data_dir: str, n_episodes: int = 10, initial_capital: float = 1000.0, save_prefix: str = "models"):
...
def evaluate_lstm_dqn(data_dir: str, n_episodes: int = 10, seq_len: int = 24, device: str = "cpu", initial_capital: float = 1000.0, save_prefix: str = "models"):
    ...
    test_env = UniswapV3DQNEnv(hourly_data, initial_capital_usd=initial_capital, mode="test")
...
def evaluate_dqn(data_dir: str, n_episodes: int = 10, device: str = "cpu", initial_capital: float = 1000.0, save_prefix: str = "models"):
...
def run_baseline_hold(data_dir: str, n_episodes: int = 10, initial_capital: float = 1000.0):
...
def run_baseline_fixed_width(data_dir: str, width: int = 5, n_episodes: int = 10, initial_capital: float = 1000.0):
...
def visualize_ppo_decisions(data_dir: str, model_path: str = "models/comparison_ppo.zip", initial_capital: float = 1000.0):
    ...
    eval_fn = make_env_fn(hourly_data, initial_capital_usd=initial_capital, mode="test")
    ...
    raw_env = UniswapV3PaperEnv(hourly_data, initial_capital_usd=initial_capital, mode="test")
    ...
    while not done:
        ...
        # Step both environments (SB3 VecEnv returns obs, rewards, dones, infos)
        obs, reward, dones, _ = vec_env.step(action)[:4]
...
def visualize_dqn_decisions(data_dir: str, model_path: str, device: str = "cpu", initial_capital: float = 1000.0):
...
def compute_pnl_custom(trajectory_data: dict, target_initial_capital: float = 1000.0, simulated_capital: float = None) -> dict:
    ...
    for name, data in trajectory_data.items():
        ...
        pnl_custom = target_initial_capital * return_pct
        end_value_custom = target_initial_capital + pnl_custom
        out[name] = {
            "initial_cap_usd": initial_cap_usd,
            "cumulative_reward": cumulative_reward,
            "return_pct": return_pct,
            "pnl_custom": pnl_custom,
            "end_value_custom": end_value_custom,
        }
    return out
...
"""
    ...
    config += "Each non-HOLD action recenters the range at current price and costs gas ($0.02).\n\n"
    ...
    if trajectory_data:
        ...
        if pnl:
            ...
            config += f"| Model | Plotted cum (USD) | Plotted return% | Plotted end ${target_cap:.0f} | Mean cum (10 ep) | Mean return% | Mean end ${target_cap:.0f} |\n"
            config += "|-------|-------------------|-----------------|-------------------|------------------|---------------|------------------|\n"
            for name, v in pnl.items():
                mean_reward = results.get("algorithms", {}).get(name, {}).get("mean_reward")
                if mean_reward is not None and initial_cap_env > 0:
                    mean_ret = mean_reward / initial_cap_env
                    mean_pnl = target_cap * mean_ret
                    mean_end = target_cap + mean_pnl
                    config += f"| {name} | {v['cumulative_reward']:.2f} | {v['return_pct']*100:.2f}% | ${v['end_value_custom']:.2f} | {mean_reward:.2f} | {mean_ret*100:.2f}% | ${mean_end:.2f} |\n"
                else:
                    config += f"| {name} | {v['cumulative_reward']:.2f} | {v['return_pct']*100:.2f}% | ${v['end_value_custom']:.2f} | - | - | - |\n"
            config += "\n(Plotted = single trajectory in the viz; Mean = average over 10 eval episodes. Env uses 2 ETH ≈ ${:.0f} at start.)\n\n".format(initial_cap_env)
...
def main():
    ...
    parser.add_argument("--initial-capital", type=float, default=1000.0, help="Initial capital in USD")
    ...
    results["baselines"]["hold"] = run_baseline_hold(args.data_dir, args.eval_episodes, initial_capital=args.initial_capital)
    results["baselines"]["fixed_width_1"] = run_baseline_fixed_width(args.data_dir, 1, args.eval_episodes, initial_capital=args.initial_capital)
    results["baselines"]["fixed_width_5"] = run_baseline_fixed_width(args.data_dir, 5, args.eval_episodes, initial_capital=args.initial_capital)
    results["baselines"]["fixed_width_10"] = run_baseline_fixed_width(args.data_dir, 10, args.eval_episodes, initial_capital=args.initial_capital)
    ...
    if not args.skip_training:
        ...
        # PPO
        try:
            run_ppo_training(args.data_dir, args.ppo_timesteps, num_envs=args.num_envs, initial_capital=args.initial_capital, save_prefix=models_dir)
        except Exception as e:
        ...
        dqn_rewards = []
        try:
            dqn_rewards = run_dqn_training(args.data_dir, args.dqn_episodes, args.device, initial_capital=args.initial_capital, save_prefix=models_dir)
        except Exception as e:
        ...
        # LSTM DQN
        try:
            run_lstm_dqn_training(args.data_dir, args.lstm_episodes, args.lstm_seq_len, args.device, initial_capital=args.initial_capital, save_prefix=models_dir)
        except Exception as e:
    ...
    try:
        results["algorithms"]["ppo"] = evaluate_ppo(args.data_dir, args.eval_episodes, initial_capital=args.initial_capital, save_prefix=models_dir)
    except Exception as e:
        print(f"PPO evaluation failed: {e}", flush=True)
        results["algorithms"]["ppo"] = {"error": str(e)}
    
    try:
        results["algorithms"]["dqn"] = evaluate_dqn(args.data_dir, args.eval_episodes, args.device, initial_capital=args.initial_capital, save_prefix=models_dir)
    except Exception as e:
    ...
    try:
        results["algorithms"]["lstm_dqn"] = evaluate_lstm_dqn(
            args.data_dir, args.eval_episodes, args.lstm_seq_len, args.device, initial_capital=args.initial_capital, save_prefix=models_dir
        )
    except Exception as e:
    ...
    if not args.no_plot:
        ...
        if pnl:
            ...
            for name, v in pnl.items():
                mean_reward = results.get("algorithms", {}).get(name, {}).get("mean_reward")
                if mean_reward is not None and initial_cap_env > 0:
                    mean_end = target_cap + target_cap * mean_reward / initial_cap_env
                    print(f"    {name:10s}  plotted: cum ${v['cumulative_reward']:+.2f}  →  end ${v['end_value_custom']:.2f}   |   mean (10ep): cum ${mean_reward:+.2f}  →  end ${mean_end:.2f}", flush=True)
                else:
                    print(f"    {name:10s}  plotted: cum ${v['cumulative_reward']:+.2f}  →  end ${v['end_value_custom']:.2f}", flush=True)
