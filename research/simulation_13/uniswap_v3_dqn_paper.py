...
def compute_L_onchain(
    ...
    """
    Compute position liquidity L in on-chain units (same as pool L from chain).
    Shared by DQN/PPO envs and compute_pnl.py; single source of truth for fee share.
    """
    if capital_usd <= 0:
        return 0.0
    lower_tick = center_tick - width * tick_spacing
    upper_tick = center_tick + width * tick_spacing
    price_raw = price_usd / (10 ** (decimals0 - decimals1))
    ...
    if price_raw <= p_lower_raw:
        value_per_L = (1.0 / sqrt_pl_raw - 1.0 / sqrt_pu_raw) / (10 ** decimals0) * price_usd
    elif price_raw >= p_upper_raw:
        value_per_L = (sqrt_pu_raw - sqrt_pl_raw) / (10 ** decimals1)
    else:
        value_per_L = (
            (1.0 / sqrt_p_raw - 1.0 / sqrt_pu_raw) / (10 ** decimals0) * price_usd
            + (sqrt_p_raw - sqrt_pl_raw) / (10 ** decimals1)
        )
    if value_per_L <= 0:
        return 0.0
...
class HourlyDataExtended:
    ...
    volatilities: Dict[pd.Timestamp, float]  # Added for PPO compatibility (e.g. NATR)
    decimals0: int
    decimals1: int
    pool_fee: float
    tick_spacing: int
    # Per-swap prices (and pool liquidity) for each hour, from swap-level data.
    # Fee share = agent_L / pool_L per swap (Uniswap v3); we scale by this to avoid over-crediting.
    swap_prices_per_hour: Optional[Dict[pd.Timestamp, np.ndarray]] = None
    swap_liquidities_per_hour: Optional[Dict[pd.Timestamp, np.ndarray]] = None  # pool L at each swap
...
def prepare_hourly_data_extended(data_dir: str) -> HourlyDataExtended:
    ...
    # Precompute per-swap prices and pool liquidity for each hour (for fee share = agent_L / pool_L).
    swaps_indexed = swaps.set_index('evt_block_time')
    swap_prices_per_hour_raw = {}
    swap_liquidities_per_hour_raw = {}
    if 'liquidity' not in swaps_indexed.columns:
        swaps_indexed['liquidity'] = np.nan  # will skip share scaling if missing
    for hour, group in swaps_indexed.groupby(pd.Grouper(freq='1h')):
        if len(group) >= 1:
            swap_prices_per_hour_raw[hour] = group['price'].values.astype(np.float64)
            liq = group['liquidity'].values.astype(np.float64)
            liq = np.where(np.isfinite(liq) & (liq > 0), liq, np.nan)  # use nan where missing/invalid
            swap_liquidities_per_hour_raw[hour] = liq
    ...
    start_idx = hourly.columns.get_loc('natr_14')
    volatilities = hourly['natr_14'].to_dict()  # Use NATR as volatility measure
    
    # Map per-swap prices and liquidities to the full (forward-filled) timestamp index
    swap_prices_per_hour = {}
    swap_liquidities_per_hour = {}
    for ts in timestamps:
        swap_prices_per_hour[ts] = swap_prices_per_hour_raw.get(ts, None)
        swap_liquidities_per_hour[ts] = swap_liquidities_per_hour_raw.get(ts, None)
    ...
    return HourlyDataExtended(
        ...
        swap_liquidities_per_hour=swap_liquidities_per_hour,
    )
...
class UniswapV3DQNEnv(gym.Env):
    ...
    def __init__(
        ...
        gas_cost_usd: float = 0.02,  # Arbitrum L2 gas cost (~$0.02 per rebalance)
        mode: str = "train",
    ...
    def _reset_state(self):
        self.idx = 0
        # self.cash removed - we use fixed self.initial_capital for every rebalance
        self.position_value = self.initial_capital
        self.accumulated_fees = 0.0
        self.accumulated_costs = 0.0  # Gas + swap paid so far (subtracted from total_value = net cash-out)
    ...
    def _compute_L_onchain(self, price_usd: float, capital_usd: float) -> float:
        """Our position's L in on-chain units (same as pool L); uses shared compute_L_onchain."""
        return compute_L_onchain(
            capital_usd,
            price_usd,
            self.hourly_data.decimals0,
            self.hourly_data.decimals1,
            self.position_center_tick,
            self.position_width,
            self.tick_spacing,
        )
    ...
    def _compute_fee(self, price_t0: float, price_t1: float) -> float:
        """
        Compute trading fee by summing over every swap in this hour.
        Fee share = our_L_onchain / pool_L (Uniswap v3); liquidity from Uniswap/chain units.
        """
        if not self.has_position or self.liquidity <= 0:
            return 0.0
        ...
        our_L = self.liquidity
        dec0 = self.hourly_data.decimals0
        dec1 = self.hourly_data.decimals1
        ...
        swap_liq = getattr(self.hourly_data, 'swap_liquidities_per_hour', None)
        liquidities = swap_liq.get(t0, None) if swap_liq else None
        
        if swap_prices is not None and len(swap_prices) >= 2:
            total_fee = 0.0
            n_swaps = len(swap_prices) - 1
            for i in range(n_swaps):
                ...
                # When we have pool_L: use on-chain units so share = our_L_onchain / pool_L is correct
                if liquidities is not None and i < len(liquidities):
                    pool_L = float(liquidities[i]) if np.isfinite(liquidities[i]) and liquidities[i] > 0 else None
                    if pool_L is not None and pool_L > 0:
                        position_value_at_p0 = self._compute_position_value(p0)
                        our_L_onchain = self._compute_L_onchain(p0, position_value_at_p0)
                        share = min(1.0, our_L_onchain / pool_L)
                        # Fee in token1 (raw): same formula as chain; then to USD
                        p0_raw = p0_c / (10 ** (dec0 - dec1))
                        p1_raw = p1_c / (10 ** (dec0 - dec1))
                        sqrt_p0_raw = math.sqrt(p0_raw)
                        sqrt_p1_raw = math.sqrt(p1_raw)
                        if sqrt_p0_raw <= sqrt_p1_raw:
                            fee_token1 = fee_mult * pool_L * (sqrt_p1_raw - sqrt_p0_raw)
                        else:
                            fee_token1 = fee_mult * pool_L * (1.0 / sqrt_p1_raw - 1.0 / sqrt_p0_raw) * p1_raw
                        fee_token1 = max(0.0, fee_token1)
                        our_fee_usd = (fee_token1 / (10 ** dec1)) * share
                        total_fee += our_fee_usd
                        continue
                # No pool_L: fallback to sim L and USD formula
                sqrt_p0_c = math.sqrt(p0_c)
                sqrt_p1_c = math.sqrt(p1_c)
                if sqrt_p0_c <= sqrt_p1_c:
                    fee_i = fee_mult * our_L * (sqrt_p1_c - sqrt_p0_c)
                else:
                    fee_i = fee_mult * our_L * (1.0 / sqrt_p1_c - 1.0 / sqrt_p0_c) * p1_c
            ...
            if p0_clamped <= p1_clamped:
                fee = fee_mult * our_L * (math.sqrt(p1_clamped) - math.sqrt(p0_clamped))
            else:
                fee = fee_mult * our_L * (1.0/math.sqrt(p1_clamped) - 1.0/math.sqrt(p0_clamped)) * p1_clamped
            return max(0.0, fee)
    ...
    def _get_obs(self) -> np.ndarray:
        ...
        # Position features (7)
        # Fixed capital model adjustments
        total_value = self.position_value
        # If we have no active LP position, we effectively hold cash
        cash_ratio = 1.0 if not self.has_position else 0.0
        
        width_normalized = self.position_width / max(self.max_width, 1)
        # Use stored position_value which is updated in step()
        position_value_ratio = self.position_value / max(self.initial_capital, 1e-10)
    ...
    def step(self, action: int):
        ...
        # Position value at step start (so we can add delta to reward = total money change)
        position_value_before = self._compute_position_value(price_t0) if self.has_position else self.initial_capital
        total_value_before = position_value_before + self.accumulated_fees - self.accumulated_costs
        
        reward = 0.0
        fee_this_step = 0.0
        cost_this_rebalance = 0.0  # Only > 0 when we actually rebalance
        ...
        if width is None:
            ...
            # Reallocate: deploy total money (position + fees − costs), then compound fees into new position.
            total_money_now = position_value_before + self.accumulated_fees - self.accumulated_costs
            swap_fee_rate = self.pool_fee
            swap_fee_cost = 0.5 * swap_fee_rate * total_money_now
            min_required = self.gas_cost + swap_fee_cost
            # Real: can't rebalance if insufficient funds (tx would fail)
            if total_money_now < min_required:
                # Force HOLD: treat rebalance as no-op (can't execute)
                if self.has_position:
                    fee = self._compute_fee(price_t0, price_t1)
                    volume_usd = self._get_volume(t0)
                    max_fee = volume_usd * self.pool_fee
                ...
                invest_amount = total_money_now - self.gas_cost - swap_fee_cost
                invest_amount = max(invest_amount, 1e-6)  # avoid zero/negative
                # Track costs for total_money; subtract from reward so agent learns rebalance is costly (HOLD preferred when position is fine)
                cost_this_rebalance = self.gas_cost + swap_fee_cost
                self.accumulated_fees = 0.0
                self.accumulated_costs += cost_this_rebalance
        ...
        # Update position value and accumulated fees (total_value = what you have when you cash out)
        self.accumulated_fees += fee_this_step
        
        # Update position value for tracking (when no position, value = initial_capital in cash)
        self.position_value = self._compute_position_value(price_t1) if self.has_position else self.initial_capital
        
        # Reward = total_value_after - total_value_before - opportunity_cost.
        # This correctly subtracts gas+swap when rebalancing (cost_this_rebalance), so agent learns HOLD is preferred when position is fine.
        total_value_after = (self.position_value if self.has_position else self.initial_capital) + self.accumulated_fees - self.accumulated_costs
        ...
        # Opportunity cost when out of range (small hourly penalty; main OOR cost is now in position value delta above)
        ...
        # Decompose for viz: use total_value delta (already includes fee, gas+swap, position change)
        # to avoid double-counting. Old formula used position_value_delta + cost_this_rebalance,
        # which double-counted gas+swap on rebalance.
        total_value_delta = total_value_after - total_value_before
        reward_positive = max(0.0, total_value_delta)
        reward_negative = opportunity_cost + max(0.0, -total_value_delta)
        
        self.idx += 1
        # Net cash-out value: position + fees − costs (gas, swap) paid so far
        total_money = (self.position_value if self.has_position else self.initial_capital) + self.accumulated_fees - self.accumulated_costs
        # Terminate when bankrupt: real LP stops when they have no funds left
        terminated = self.idx >= self.n_steps or total_money <= 0
        info = {
            "price": price_t1,
            "total_value": total_money,  # Net money when you cash out (position + fees − costs)
            "accumulated_costs": self.accumulated_costs,  # Rebalance cost so far (gas + swap)
...
def train_lstm_dqn(
    ...
    # Create environments
    train_env = UniswapV3DQNEnv(hourly_data, initial_capital_usd=initial_capital_usd, mode="train")
    eval_env = UniswapV3DQNEnv(hourly_data, initial_capital_usd=initial_capital_usd, mode="eval")
    ...
    print(f"  Steps per episode (train): {train_env.n_steps}")
    print()
    ...
    for episode in range(n_episodes):
        ...
        # Logging (episode reward = sum over all steps in train period; show per-step for interpretability)
        if (episode + 1) % 10 == 0:
            avg_reward = np.mean(episode_rewards[-10:])
            n_steps_per_ep = train_env.n_steps
            avg_per_step = avg_reward / n_steps_per_ep if n_steps_per_ep else 0.0
            print(f"  Episode {episode+1}/{n_episodes} | Avg Reward: {avg_reward:.2f} ({avg_per_step:.4f}/step, {n_steps_per_ep} steps) | Epsilon: {agent.epsilon:.3f}", flush=True)
...
def train_dqn(
    data_dir: str,
    n_episodes: int = 500,  # 500 episodes * ~2200 steps = ~1.1M steps
    initial_capital_usd: float = 1000.0,
    max_steps_per_episode: Optional[int] = None,
