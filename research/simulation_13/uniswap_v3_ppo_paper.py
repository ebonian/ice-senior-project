...
def _compute_L_onchain_from_bounds(
    capital_usd: float,
    price_usd: float,
    decimals0: int,
    decimals1: int,
    ...
    """Thin wrapper: derive center_tick and width from bounds, call shared formula."""
    from uniswap_v3_dqn_paper import compute_L_onchain
    lower_tick = price_to_tick(price_lower)
    upper_tick = price_to_tick(price_upper)
    center_tick = (lower_tick + upper_tick) // 2
    width = max(1, (upper_tick - lower_tick) // (2 * tick_spacing))
    return compute_L_onchain(
        capital_usd, price_usd, decimals0, decimals1,
        center_tick, width, tick_spacing,
    )
...
class HourlyData:
    ...
    swap_liquidities_per_hour: Optional[Dict[pd.Timestamp, np.ndarray]] = None
...
def prepare_hourly_data(data_dir: str) -> HourlyData:
    ...
    if 'liquidity' not in swaps_indexed.columns:
        swaps_indexed['liquidity'] = np.nan
    for hour, group in swaps_indexed.groupby(pd.Grouper(freq='1h')):
        if len(group) >= 1:
            ...
            liq = np.where(np.isfinite(liq) & (liq > 0), liq, np.nan)
...
class UniswapV3PaperEnv(gym.Env):
    ...
    def _compute_L_onchain(self, price_usd: float, capital_usd: float,
                           price_lower: float, price_upper: float) -> float:
        """Our position's L in on-chain units (same as pool L); uses shared compute_L_onchain."""
        return _compute_L_onchain_from_bounds(
            capital_usd, price_usd,
            self.hourly_data.decimals0, self.hourly_data.decimals1,
            price_lower, price_upper, self.tick_spacing,
        )
    ...
    def _compute_fee(self, price_t: float, price_t1: float, L: float, price_lower: float, price_upper: float) -> float:
        ...
        if swap_prices is not None and len(swap_prices) >= 2:
            ...
            for i in range(n_swaps):
                ...
                if liquidities is not None and i < len(liquidities):
                    ...
                    if pool_L is not None and pool_L > 0:
                        position_value_at_p0 = self._compute_position_value(p0, price_lower, price_upper, L)
                        our_L_onchain = self._compute_L_onchain(p0, position_value_at_p0, price_lower, price_upper)
            ...
            if p_t_clamped <= p_t1_clamped:
                fee = fee_mult * our_L * (math.sqrt(p_t1_clamped) - math.sqrt(p_t_clamped))
            else:
                fee = fee_mult * our_L * (1.0 / math.sqrt(p_t1_clamped) - 1.0 / math.sqrt(p_t_clamped)) * p_t1_clamped
            return max(0.0, fee)
    ...
    def step(self, action: int):
        ...
        if self.has_lp and self.liquidity > 0 and self.lp_lower_tick is not None and self.lp_upper_tick is not None:
            pv_lower = tick_to_price(self.lp_lower_tick)
            pv_upper = tick_to_price(self.lp_upper_tick)
            position_value_before = self._compute_position_value(price_t0, pv_lower, pv_upper, self.liquidity)
        else:
            position_value_before = self.initial_capital
        ...
        if tick_width == 0:
            ...
            # Compound: deploy total money (position + fees − costs), then pay gas and swap from it.
            ...
            min_required = self.gas_cost_usd + swap_fee_cost
            ...
            if total_money_now < min_required:
                ...
                invest_amount = total_money_now - self.gas_cost_usd - swap_fee_cost
                invest_amount = max(invest_amount, 1e-6)
                ...
                self.initial_value_usd = invest_amount
                self.has_lp = True
                ...
                fee = min(fee, max_fee)
                fee_this_step = fee
                cost_this_rebalance = self.gas_cost_usd + swap_fee_cost
                self.accumulated_fees = 0.0
                self.accumulated_costs += cost_this_rebalance
        
        # Compute position_value_after for total_value and reward
        if self.has_lp and self.liquidity > 0 and self.lp_lower_tick is not None and self.lp_upper_tick is not None:
            pv_lower = tick_to_price(self.lp_lower_tick)
            pv_upper = tick_to_price(self.lp_upper_tick)
            position_value_after = self._compute_position_value(price_t1, pv_lower, pv_upper, self.liquidity)
        else:
            position_value_after = self.initial_capital
        ...
        total_value_after = (position_value_after if self.has_lp else self.initial_capital) + self.accumulated_fees - self.accumulated_costs
        ...
        if self.has_lp and self.lp_lower_tick is not None:
            ...
            if not in_range:
                # Use current position value (not initial) — capital stuck OOR can have changed
                opp_value = position_value_after if self.has_lp else self.initial_capital
                opportunity_rate = 0.0000057  # 5% APY / 8760 hours
                opportunity_cost = opp_value * opportunity_rate
        reward -= opportunity_cost
        
        # Net cash-out value: position + fees − costs (gas, swap) paid so far
        total_money = (position_value_after if self.has_lp else self.initial_capital) + self.accumulated_fees - self.accumulated_costs
