    ...
    def step(self, action: int):
        ...
        if width is None:
            ...
            if self.has_position:
                ...
                # Note: We do NOT add fee to capital (no compounding)
        else:
            # Reallocate with width, always centered at current price
            
            # Gas cost
            # In fixed capital model, gas is a direct cost penalty
            reward -= self.gas_cost
            
            # Swap fee cost: ~50% of position needs swapping during rebalance (heuristic)
            ...
            # Invest FIXED initial capital into position (no compounding)
            invest_amount = self.initial_capital
            ...
            # Collect fees for this step (immediately after rebalancing)
            fee = self._compute_fee(price_t0, price_t1)
        ...
        # Update position value for tracking
        # In fixed capital model, we track value relative to initial capital
        ...
        if self.has_position and not in_range:
            ...
            # Opportunity cost based on current position value
        ...
        return self._get_obs(), reward, terminated, False, {
            "price": price_t1,
            "total_value": self.position_value,  # Track current position value
            "in_range": in_range,
            "action_width": width,
        }
